...
static void event_handler(void* arg, esp_event_base_t event_base, int32_t event_id, void* event_data) {
    esp_netif_t* event_netif = (esp_netif_t*)arg;
    if (event_netif != netif) {
        return;  // Ignore events for other netifs
    }
    ...
    if (event_base == WIFI_EVENT) {
        switch (event_id) {
            case WIFI_EVENT_STA_START:
                ...
                if (!m_is_scanning) {  // Connect only if we're not scanning the WiFi.
                    esp_wifi_connect();
                }
                break;
...
static void wifi_connection_init_internal(void) {
    ...
    ESP_ERROR_CHECK(esp_event_handler_register(WIFI_EVENT, ESP_EVENT_ANY_ID, &event_handler, (void*)netif));
    ESP_ERROR_CHECK(esp_event_handler_register(IP_EVENT, ESP_EVENT_ANY_ID, &event_handler, (void*)netif));
}
